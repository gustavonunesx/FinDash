...
- [ ] **Meta mensal de economia** — usuário define quanto quer guardar por mês; barra de progresso no dashboard mostra quanto falta. Simples: coluna `meta_economia_mensal` em `configuracoes`.
- [ ] **Gastos recorrentes** — marcar gasto como recorrente (flag + dia do mês); cron no Supabase sugere adicionar automaticamente. Reduz atrito de uso diário.
